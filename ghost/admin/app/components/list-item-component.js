...
export default class ListItemComponent extends Component {
    ...
    @action
    goToLexicalEditor() {
        this.router.transitionTo('lexical-editor.new', 'post');
    }

    @task
    *generateSlugTask(item) {
        try {
            ...
            const slug = dasherize(item.title);
            ...
            const post = this.store.createRecord('post', {
                        ...
                        children: [
                            {
                                children: [
                                    {
                                        markdown: item.generated_output,
                                        type: 'markdown',
                                        version: 1
                                    }
                                ],
                ...
                html: `<p>${item.generated_output}</p>`,
                plaintext: item.generated_output,
                createdAtUTC: now,
                updatedAtUTC: now,
            ...
            yield post.save();
            this.notifications.showNotification('Post created successfully', {type: 'success'});
        } catch (error) {
            this.notifications.showNotification('Error creating post', {type: 'error'});
        }
    }
}